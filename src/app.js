...
function renderCstLinks() {
  ...
  target.innerHTML = csts
    .map((item) => {
      return `
        <a class="cst-link" href="cst.html?cst=${encodeURIComponent(item.cst)}">
...
function renderSummary(rows) {
  ...
  const primaryYouth = sumBy(rows, "primaryYouth");
  const primaryYoY = sumBy(rows, "primaryYoY");
  ...
  const committeeRate = ships
    ...
    : 0;
  const avgMetric = ships
    ? (sumBy(rows, "unitMetric") / ships).toFixed(2)
    : "0.00";
  ...
  setText("primaryYouth", fmt.format(primaryYouth));
  ...
  byId("primaryDelta").textContent = deltaText(primaryYoY);
  byId("primaryDelta").className = deltaClass(primaryYoY);
...
function bindControls() {
  ...
  byId("expandAll").addEventListener("click", () => {
    const rows = state.data.ships.filter(matchesFilters);
    rows.forEach((ship) => {
      const cst = String(ship.cst || "Unassigned");
      state.openCsts.add(cst);
      state.openCouncils.add(`${cst}|${ship.council || "Unassigned"}`);
    });
    render();
  });
  byId("groupRows").addEventListener("click", (event) => {
    ...
    const cstButton = event.target.closest("[data-cst]");
    if (shipButton) {
    ...
    if (cstButton) {
      const cst = cstButton.dataset.cst;
      if (state.openCsts.has(cst)) {
        state.openCsts.delete(cst);
        state.selectedId = null;
      } else {
        state.openCsts.add(cst);
      }
...
async function init() {
  ...
  populateSelect("ownerFilter", state.data.ships.map((ship) => ship.owner));
  populateSelect("auxFilter", state.data.ships.map((ship) => ship.auxDistrict));
  populateSelect("focusFilter", state.data.ships.map((ship) => ship.focus));
  renderCstLinks();
  bindControls();
  render();
}
